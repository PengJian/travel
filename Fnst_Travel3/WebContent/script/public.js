...
function openWin(f,n,w,h,s){
	...
	l = (screen.width - w)/2;
	t = (screen.height - h)/2;
	sFeatures = "dialogHeight="+ h +"px;dialogWidth="+ w
			+ "px;scroll=" + sb + ";resizable:no;help:yes;status:no;scroll:no";
	value = window.showModalDialog (f , n , sFeatures );
	if(value == "refresh"){
...
//for select yearth
function openWin3(f,n,w,h,s){
	sb = s == "1" ? "1" : "0";
	l = (screen.width - w)/2;
	t = (screen.height - h)/2;
	sFeatures = "dialogHeight="+ h +"px;dialogWidth="+ w
			+ "px;scroll=" + sb + ";resizable:no;help:yes;status:no;scroll:no";
	value = window.open(f , n , sFeatures );
	if(value == "refresh"){
		 window.location.reload();
	}
 ...
 * 重设密码
 */
function reset(url,info){
	if(openDeleteDialog(url,"确定要重设密码？")){
		window.location.reload(true);
	}
}
