...
<center>
      <TABLE width="880" border=0 cellPadding=0 cellSpacing=0 borderColor=#ffffff bgColor=#dee7ff style="FONT-SIZE: 10pt">
        <TBODY>
          ...
          bgColor=#dee7ff><FONT color=#f7f7f7>用户记录查询<font color="#FFFFFF">&nbsp;</font></FONT> </TD>
        ...
        <TBODY>
          ...
          <TR>
            <TD height=14 align=left vAlign=center noWrap>
            	<form style="margin-left:10px" action="<%=path %>/back/selectUserLog.action" method="post">
            	 检索条件(姓名、工号、或者IP)：<input name="str" width="200" value="${str}" type="text">
      ...
      <table width="880" border="0" cellPadding="0" cellSpacing="1" bgcolor="#6386d6">
	      ...
	      <td width="40%" height="37" align="center"><b>用户信息(工号：姓名：IP)</b></td>
		  <td width="30%" height="37" align="center"><B>操作详情</B></td>
		  <td width="20%" height="37" align="center"><B>操作时间</B></td>
          </tr>
       
          <!-- 列表数据栏 -->
         <c:if test="${!empty pm.datas}">
          <c:forEach items="${pm.datas }" var="log" varStatus="status">
	      <tr bgcolor="#EFF3F7" class="TableBody1" onmouseover="this.bgColor = '#DEE7FF';" onmouseout="this.bgColor='#EFF3F7';">
	          <td align="center" vAlign="center">${status.count}</td>
	          <td align="center" vAlign="center">${log.userInfo}</td>
	          <td align="center" vAlign="center">${log.action}</td>
	           <td align="center" vAlign="center">${log.date}</td>
        </tr>
        </c:forEach>
		</c:if>
        <!-- 在列表数据为空的时候，要显示的提示信息 -->
      ...
      <TABLE width="880" border=0 align=center cellPadding=0 cellSpacing=0 borderColor=#ffffff style="FONT-SIZE: 10pt">
        <TBODY>
          <TR>
            ...
            <!-- 可以在这里插入分页导航条 -->
   <pg:pager url="../back/selectUserLog.action" items="${pm.total}"  maxPageItems="${pageSize}"  export="currentPageNumber=pageNumber">
   <pg:param name="str" value="${str}"/>
	<pg:first>
		<a href="${pageUrl}">首页</a>
	</pg:first>
	<pg:prev>
